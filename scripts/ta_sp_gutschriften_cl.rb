...
class KontoauszugVerarbeiter
  ...
  def initialize(verzeichnis)
    ...
    @transaktionen_nach_datei = {} # Speichert welche Transaktionen aus welcher Datei kommen
    @ks = CustomerSearcher.new(FN)
  end
  
  # Entferne nicht mehr benötigte Methoden
  # lade_bekannte_transaktionen und speichere_bekannte_transaktionen entfernt
  ...
  def neue_dateien_vorhanden?
    ...
    zeitstempel_mtime = File.mtime(@zeitstempel_datei)
  ...
  def dateien_verarbeiten
    ...
    # Minimale ZIP-Dateien für die letzten 3 Monate auswählen
    selected_zip_files = select_minimal_recent_files(3)
    
    # Sortiere Dateien nach Datum (neueste zuerst) um zu wissen welche die neueste ist
    sorted_files = selected_zip_files.sort_by do |datei|
      File.basename(datei).match(/(\d{8})-(\d{8})/)[2] # End-Datum
    end.reverse
    
    neueste_datei = sorted_files.first
    puts "Neueste Datei: #{File.basename(neueste_datei)}" if neueste_datei
    
    selected_zip_files.each do |zip_datei|
      puts zip_datei
      ist_neueste = (zip_datei == neueste_datei)
      zip_datei_verarbeiten(zip_datei, ist_neueste)
    end
    
    # Auch XML-Dateien direkt im Verzeichnis verarbeiten
    Dir.glob(File.join(@verzeichnis, "*.xml")).each do |xml_datei|
      xml_datei_verarbeiten(xml_datei, true) # XML-Dateien als "neueste" behandeln
    end
    
    # Erkenne unique Transaktionen aus der neuesten Datei
    erkenne_unique_transaktionen
  ...
  def erkenne_unique_transaktionen
    puts "DEBUG: Starte Unique-Erkennung mit #{@transaktionen.length} Transaktionen"
    
    # Zähle wie oft jede Transaktion vorkommt
    transaktions_count = Hash.new(0)
    transaktions_mapping = {}
    
    @transaktionen.each_with_index do |transaktion, index|
      schluessel = erstelle_transaktions_schluessel(transaktion)
      transaktions_count[schluessel] += 1
      transaktions_mapping[schluessel] = transaktion
      
      puts "DEBUG #{index}: #{transaktion[:buchungsdatum]} | #{transaktion[:betrag]} | #{transaktion[:aus_neuester_datei] ? 'NEUESTE' : 'ÄLTERE'} | #{schluessel[0..50]}..."
    end
    
    puts "DEBUG: Gefundene unique Schlüssel: #{transaktions_count.keys.length}"
    
    # Entferne Duplikate und markiere unique Transaktionen aus neuester Datei
    unique_transaktionen = []
    neue_count = 0
    
    transaktions_count.each do |schluessel, count|
      transaktion = transaktions_mapping[schluessel]
      
      puts "DEBUG: Schlüssel #{schluessel[0..50]}... | Count: #{count} | Aus neuester: #{transaktion[:aus_neuester_datei]}"
      
      if count == 1 && transaktion[:aus_neuester_datei]
        # Einmalig UND aus neuester Datei = wirklich neu
        transaktion[:ist_neu] = true
        neue_count += 1
        puts "  -> MARKIERT ALS NEU"
      else
        # Entweder Duplikat oder nicht aus neuester Datei
        transaktion[:ist_neu] = false
        puts "  -> Nicht neu (Count: #{count}, Aus neuester: #{transaktion[:aus_neuester_datei]})"
      end
      
      # Füge nur einmal hinzu (unique)
      unique_transaktionen << transaktion
    end
    
    @transaktionen = unique_transaktionen
    puts "DEBUG: #{neue_count} unique Transaktionen aus neuester Datei gefunden"
    puts "DEBUG: #{@transaktionen.length} Transaktionen total (nach Duplikat-Entfernung)"
    
    # Zusätzliche Prüfung
    tatsaechlich_neue = @transaktionen.count { |t| t[:ist_neu] }
    puts "DEBUG: Tatsächlich als neu markierte Transaktionen: #{tatsaechlich_neue}"
  end
  
  def erstelle_transaktions_schluessel(transaktion)
    [
      transaktion[:buchungsdatum],
      transaktion[:betrag],
      transaktion[:ueberweisungsinformationen],
      transaktion[:zahler_empfaenger]
    ].join('|')
  end
  
  def zip_datei_verarbeiten(zip_datei, ist_neueste_datei = false)
    Zip::File.open(zip_datei) do |zip|
      zip.each do |eintrag|
        ...
        gutschriften_verarbeiten(doc, ist_neueste_datei)
      end
    end
  end
  
  def xml_datei_verarbeiten(xml_datei, ist_neueste_datei = false)
  ...
  def gutschriften_verarbeiten(doc, ist_neueste_datei = false)
    ns = { 'camt' => NAMESPACE }
      ...
      if tx_details.any?
        tx_details.each do |tx|
          ...
          transaktion = {
            ...
            rui: rui,
            aus_neuester_datei: ist_neueste_datei,
            ist_neu: false # Wird später in erkenne_unique_transaktionen gesetzt
          }
          
          # Erstelle einen eindeutigen Schlüssel für die Transaktion
          transaktions_schluessel = erstelle_transaktions_schluessel(transaktion)
          
          # Füge die Transaktion immer hinzu (Duplikate werden später behandelt)
